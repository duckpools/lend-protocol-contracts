```scala
{
	...
	val desiredToken  = SELF.R7[Coll[Byte]].get
	...
	if (OUTPUTS.size < 3) {
		...
		val validTokens = if (exchangedTokens.size > 0){exchangedTokens(0)._1 == desiredToken && exchangedTokens(0)._2 >= requestAmount} else {false}
