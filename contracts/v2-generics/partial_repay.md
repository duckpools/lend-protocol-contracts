```scala
{
	...
	sigmaProp(if (OUTPUTS.size <= 2) {
		...
		val collateralTokens = currentCollateralBox.tokens.slice(1,currentCollateralBox.tokens.size)
		...
		val successorTokens = successorCollateralBox.tokens.slice(1,successorCollateralBox.tokens.size)
		...
		val isPartialRepayment = (
			...
			retainBorrowTokenId &&
			validBorrowTokens &&
			collateralTokens == successorTokens
		)
		isPartialRepayment
	})
}
